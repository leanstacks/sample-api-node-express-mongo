...
describe('GET /v1/accounts/:id', () => {
  ...
  it('should require authentication', async () => {
    const res = await request(app).get('/v1/accounts/1').set('Accept', 'application/json');

    expect(res.statusCode).toEqual(401);
  });

  it('should return status code 200', async () => {
    mockedAccountService.findOne.mockResolvedValue(accountData);
  ...
  it('should return status code 500 when an error occurs', async () => {
    mockedAccountService.findOne.mockRejectedValue(new Error());
  ...
  it('should call AccountService to find an account', async () => {
    mockedAccountService.findOne.mockResolvedValue(accountData);
    ...
    expect(mockedAccountService.findOne).toHaveBeenCalledWith('1');
  });
});
